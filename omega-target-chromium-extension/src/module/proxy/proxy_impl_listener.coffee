...
class ListenerProxyImpl extends ProxyImpl
  ...
  proxyInfo: (proxy, auth, proxyDNS) ->
    ...
    # Resolving names through the proxy is the safer default (it avoids
    # leaking the hostname to the local resolver), but it breaks setups that
    # do their own resolution, e.g. DNS-over-HTTPS - so let the profile turn
    # it off. Absent an explicit choice, keep the old always-on behavior.
    if proxyInfo.type == 'socks' and proxyDNS != false
